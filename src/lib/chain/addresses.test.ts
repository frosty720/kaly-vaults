...
describe('addresses', () => {
	it('testnet returns the v2 deployed proxies + KLC-pair stables', () => {
		...
		expect(Object.keys(a.stables)).toEqual(['KUSD', 'USDT', 'USDC']);
		expect(a.stables.USDT.decimals).toBe(6);
		expect(a.stables.KUSD.decimals).toBe(18);
		expect(a.stables.USDC.decimals).toBe(6);
		expect(a.stables.USDC.address).toBe('0x148d19609F3Ad595F8455225510f89cF0F121013');
	});
	it('mainnet returns the deployed v4 proxies + USDT/KUSD/USDC stables', () => {
		...
		expect(a.stables.KUSD.decimals).toBe(18);
		expect(a.stables.USDC.decimals).toBe(6);
		expect(a.stables.USDC.address).toBe('0x9cAb0c396cF0F4325913f2269a0b72BD4d46E3A9');
